Ext.require([
	'Module.pos.transferOrderIn.store.TransferOrderInLines',
...
Ext.define('Module.pos.transferOrderIn.view.CreateWindow', {
	extend: 'Ext.Window',
	alias: 'widget.transferOrderInCreateWindow',
	height: 540,
	width: 800,
	constrain: true,
	modal: true,
	...
	apiPath: Ext.String.format('{0}/api/TransferOrderInsApi', "/" === basket.contextPath ? "" : basket.contextPath),
	...
	initComponent: function() {
		...
		var store = me.store = Ext.create('Module.pos.transferOrderIn.store.TransferOrderInLines', { 
			listeners: {
		...
		var grid = me.grid = Ext.create('Ext.grid.Panel', {
			...
			columns: [
				...
				{ 
				    text: '应收数量', dataIndex: 'DemandQuantity', width: 100,
			    ...
			    { text: '待收数量', dataIndex: 'LaveQuantity', width: 100 }
			],
			listeners: {
	...
	onAddLineClick: function(btn, event, eOpts){
		var me = this,
			store = this.getStore();
		if(store){
			var model = Ext.create('Module.pos.transferOrderIn.model.TransferOrderInLine', {
				SKU: '',
				Name: '',
				UOM: '',
				Quantity: 0.0
	...
	onCloseOrderClicked: function (button, event) {
		...
		Ext.Msg.confirm('确认', '您是否确认关闭调拨入库单?', 
			function(btn, text){
	...
	onConfirmClicked: function(button, event){
		...
		Ext.Msg.confirm('确认', '您是否确认调拨入库单正确?', 
			function(btn, text){
				if (btn == 'ok' || btn == 'yes' || btn == '确定'){
					Ext.Ajax.request({
	...
	onCancelClicked: function(button, event){
		...
		Ext.Msg.confirm('确认', '您是否确定作废调拨入库单?', 
			function(btn, text){
				if (btn == 'ok' || btn == 'yes' || btn == '确定'){
					Ext.Ajax.request({
	...
	onReceiveClicked: function (button, event) {
	    var me = this;
	    if (!me.createShipWindow) {
	        me.createShipWindow = Ext.widget('transferOrderInShipWindow');
	    }
	...
	refresh: function(data){
		...
		try{
			...
			var record = Ext.create('Module.pos.transferOrderIn.model.TransferOrderIn', data);
...
Ext.define('Module.pos.transferOrderIn.view.shipWindow', {
    extend: 'Ext.Window',
    alias: 'widget.transferOrderInShipWindow',
    height: 540,
    width: 800,
    constrain: true,
    modal: true,
    ...
    initComponent: function () {
        ...
        var grid = me.grid = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                { text: '待收数量', dataIndex: 'LaveQuantity', width: 100 },
                {
                    text: '本次收货数量', dataIndex: 'ShipQuantity', width: 100,
                    xtype: 'numbercolumn', format: '0,000.00',
                    align: 'right',
                    editor: {
                        xtype: 'ux.field.NumericField',
    ...
    onConfirmShipClicked: function (button, evet) {
        ...
        Ext.Ajax.request({
            ...
            url: Ext.String.format("{0}/{1}/{2}", me.apiPath, objectId, 'Receive'),
            method: 'POST',
            jsonData: submitValues,
            success: function (response, opts) {
                var obj = Ext.decode(response.responseText);
